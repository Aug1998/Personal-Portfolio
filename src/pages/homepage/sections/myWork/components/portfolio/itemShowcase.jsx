...
class ItemShowcase extends Component {
  ...
  handleFilter = (filter) => {
    ...
    var filterPageCount;
    if (filter !== "") {
      filterPageCount = Math.ceil(
        this.state.items.filter((item) => item.desc === filter).length /
  ...
  handlePageChange = (clickedPage) => {
    ...
    $(".paginationContainer ul li button").removeClass("pagActive");
    $(
      ".paginationContainer ul li:nth-of-type(" + clickedPage + ") button"
    ).addClass("pagActive");
  ...
  handleNextPage = () => {
    ...
    $(".paginationContainer ul li button").removeClass("pagActive");
    $(
      ".paginationContainer ul li:nth-of-type(" + nextPage + ") button"
    ).addClass("pagActive");
  ...
  handlePrevPage = () => {
    ...
    $(".paginationContainer ul li button").removeClass("pagActive");
    $(
      ".paginationContainer ul li:nth-of-type(" + prevPage + ") button"
    ).addClass("pagActive");
  ...
  updateItemEvents = () => {
    ...
    if ($(window).width() > 1024) {
      $(".portfolio_item").on("mouseover", function () {
        $(this)
          .find(".portfolio_item_info")
          .addClass("portfolio_item_info_active");
      });

      $(".portfolio_item").on("mouseout", function () {
        $(this)
          .find(".portfolio_item_info")
          .removeClass("portfolio_item_info_active");
      });
    } else {
  ...
  render() {
    ...
    $(window).on("load", function () {
      $(".paginationContainer ul li:nth-of-type(1) button").addClass(
        "pagActive"
      );
    });
    ...
    return (
      <div className="portfolio_wrapper">
        <div className="portfolio_nav">
          <PortfolioButton
            text="Todo"
            className={this.state.keyword === "" ? "active" : ""}
          ...
          <PortfolioButton
            text="Web a medida"
            id="btn_webdev"
            className={this.state.keyword === "Webdev" ? "active" : ""}
            onFilter={() => this.handleFilter("Webdev")}
          />

          <PortfolioButton
            text="WordPress"
            id="btn_wp"
            className={this.state.keyword === "WordPress" ? "active" : ""}
            onFilter={() => this.handleFilter("WordPress")}
          />

          <PortfolioButton
            text="Arreglos"
            id="btn_arreglos"
            className={this.state.keyword === "Arreglos" ? "active" : ""}
            onFilter={() => this.handleFilter("Arreglos")}
          />
        </div>

        <div className="portfolio_content">{this.renderPortfolio()}</div>
